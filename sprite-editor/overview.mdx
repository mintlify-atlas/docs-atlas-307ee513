---
title: "Sprite Editor: CPC Pixel Art in the Browser"
sidebarTitle: "Overview"
description: "Tour the Webtile sprite editor's layout, canvas rendering, zoom levels, frame strip, and right-sidebar palette and minimap controls."
---
...
| Right sidebar | 200 px (fixed) | Minimap, ink slots, palette picker, frame strip |
...
- **Frame strip** (in the bottom section) — thumbnails of every frame, an **+** button to clone the current frame, and a ✕ delete button on hover.
- **Animation playback** — a looping canvas preview that cycles through all frames at the configured FPS.
...
<CardGroup cols={3}>
  ...
  <Card title="Export (EXPORT)" icon="download">
    Generate CPC hardware bytes as Z80 ASM `DEFB` or Locomotive BASIC `DATA` statements. Supports hex and decimal output, optional mask interleaving, and CPC scanline-interleaved row ordering.
  </Card>
</CardGroup>
